...
void main() {
  group('HeuristicSchedulingEngine', () {
    ...
    test(
      'low energy penalizes high-load tasks (tends to schedule them later)',
      () {
        final engine = HeuristicSchedulingEngine();
        final day = DateTime(2026, 1, 1);
    ...
    test(
      'tuning highLoadPenaltyWhenLowEnergy can push high-load work out of mornings',
      () {
        final engine = HeuristicSchedulingEngine();
        final day = DateTime(2026, 1, 1);
    ...
    test('blocks missing dependencies and reports blocked ids', () {
      final plan = HeuristicSchedulingEngine().plan(
        SchedulingRequest(
    ...
    test(
      'honors earliest start and does not fallback hard deadlines past due',
      () {
        final plan = HeuristicSchedulingEngine().plan(
          SchedulingRequest(
            day: DateTime(2026, 6, 14),
            tasks: [
              PlanTask(
                ...
                load: CognitiveLoad.low,
                tag: 'Task',
                earliestStart: DateTime(2026, 6, 14, 9),
              ),
              PlanTask(
                id: 'hard',
                title: 'Hard',
        ...
        final issue = plan.issues.singleWhere(
          (issue) => issue.taskId == 'hard',
        );
