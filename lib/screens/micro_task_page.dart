...
class _MicroTaskPageState extends State<MicroTaskPage> {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx2, setInner) => AlertDialog(
          title: const Text('导入清单', style: TextStyle(fontWeight: FontWeight.bold)),
          ...
          content: ConstrainedBox(
            ...
            child: SingleChildScrollView(
              child: Column(
                ...
                children: [
                  const Text('粘贴清单或场景文本以导入任务。', style: TextStyle(color: Colors.grey)),
                  ...
                  if (preview != null) ...[
                    ...
                    Wrap(
                      ...
                      children: [
                        ...
                        Chip(label: Text('+${preview!.totalPoints} 积分', style: const TextStyle(color: Colors.green, fontWeight: FontWeight.bold))),
                      ],
                    ),
                    const SizedBox(height: 8),
                    SizedBox(
  ...
  void _showAddMicroTaskDialog(BuildContext context) {
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx2, setInner) => AlertDialog(
          title: Text(AppStrings.of(context, 'micro_dialog_add'), style: const TextStyle(fontWeight: FontWeight.bold)),
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
  ...
  void _showEditMicroTaskDialog(BuildContext context, MicroTask task) {
    ...
    showDialog(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx2, setInner) => AlertDialog(
          title: Text(AppStrings.of(context, 'micro_dialog_edit'), style: const TextStyle(fontWeight: FontWeight.bold)),
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
          content: Column(
            mainAxisSize: MainAxisSize.min,
            children: [
  ...
  Future<void> _batchSchedule() async {
    ...
    final ok = await showDialog<bool>(
      ...
      builder: (ctx) => StatefulBuilder(
        builder: (ctx2, setInner) => AlertDialog(
          title: Text(AppStrings.of(context, 'micro_schedule_dialog_title'), style: const TextStyle(fontWeight: FontWeight.bold)),
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(20)),
          content: SingleChildScrollView(
            child: Column(
              ...
              children: [
                ...
                const SizedBox(height: 16),
                ListTile(
                  contentPadding: EdgeInsets.zero, title: const Text('日期'), trailing: const Icon(Icons.calendar_today, size: 18),
                  subtitle: Text('${day.year}-${day.month.toString().padLeft(2, '0')}-${day.day.toString().padLeft(2, '0')}', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                  onTap: () async {
                    final now = DateTime.now();
                ...
                ListTile(
                  contentPadding: EdgeInsets.zero, title: const Text('时间'), trailing: const Icon(Icons.access_time, size: 18),
                  subtitle: Text(time.format(context), style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16)),
                  onTap: () async {
  ...
  Widget _buildPriorityBadge(MicroTask task) {
    ...
    final colors = [Colors.grey, Colors.blue, Colors.teal, Colors.orange, Colors.red];
    ...
    return Container(
      ...
      child: Text('P$p', style: TextStyle(fontSize: 10, fontWeight: FontWeight.w800, color: color)),
    );
  }
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: isDark 
            ...
            : [theme.colorScheme.primaryContainer.withOpacity(0.8), theme.colorScheme.primaryContainer.withOpacity(0.3)],
          begin: Alignment.topLeft, end: Alignment.bottomRight,
        ),
        borderRadius: BorderRadius.circular(24),
        boxShadow: isDark ? [] : [BoxShadow(color: theme.colorScheme.primary.withOpacity(0.15), blurRadius: 20, offset: const Offset(0, 10))],
      ),
      child: Column(
        crossAxisAlignment: CrossAxisAlignment.start,
        children: [
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              Text(AppStrings.of(context, 'micro_ai_suggestion'), style: TextStyle(fontSize: 16, fontWeight: FontWeight.w800, color: theme.colorScheme.onSurface)),
              ElevatedButton.icon(
          ...
          Row(
            ...
            children: [
              _buildHeaderStatItem('已完成', '$doneCount/${_tasks.length}', Icons.check_circle_rounded),
              _buildHeaderStatItem('已获积分', '$completedPoints', Icons.stars_rounded),
  ...
  Widget build(BuildContext context) {
    ...
    final selectedPoints = _selectedTasks().where((t) => !t.done).fold<int>(0, (sum, task) => sum + _pointsFor(task));
    ...
    return Scaffold(
      backgroundColor: Theme.of(context).brightness == Brightness.dark ? const Color(0xFF121212) : const Color(0xFFF2F2F7),
      appBar: AppBar(
        title: Text(AppStrings.of(context, 'micro_title'), style: const TextStyle(fontWeight: FontWeight.bold)),
        backgroundColor: Colors.transparent,
        elevation: 0,
        actions: [
          IconButton(icon: const Icon(Icons.refresh_rounded), onPressed: _loadMicroTasks),
          IconButton(
            tooltip: _batchMode ? '退出批量' : '批量模式',
            icon: Icon(_batchMode ? Icons.close_rounded : Icons.checklist_rounded),
            onPressed: () => _setBatchMode(!_batchMode),
          ),
          IconButton(tooltip: '导入清单', icon: const Icon(Icons.upload_file_rounded), onPressed: _showImportMicroTasksDialog),
              ...
              children: [
                ...
                Expanded(
                  child: ListView.builder(
                    ...
                    itemBuilder: (context, index) => _buildMicroTaskBubble(_tasks[index]),
                  ),
                ),
              ],
            ),
      ...
      floatingActionButton: _batchMode
          ...
          : FloatingActionButton.extended(
              ...
              label: Text(AppStrings.of(context, 'micro_btn_add'), style: const TextStyle(fontWeight: FontWeight.bold)),
            ...
            child: Row(
              ...
              children: [
                Column(
                  ...
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Text('已选 $count 项', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 15)),
                    Text('可获 +$points ${_pointsUnit(context)}', style: TextStyle(color: Theme.of(context).colorScheme.primary, fontSize: 12, fontWeight: FontWeight.bold)),
                  ],
                ),
                Row(
                  mainAxisSize: MainAxisSize.min,
                  children: [
                    IconButton(icon: const Icon(Icons.delete_outline, color: Colors.redAccent), onPressed: count == 0 ? null : _batchDelete),
                    const SizedBox(width: 4),
                    ElevatedButton(onPressed: count == 0 ? null : _batchSchedule, style: ElevatedButton.styleFrom(backgroundColor: Theme.of(context).colorScheme.tertiary, foregroundColor: Theme.of(context).colorScheme.onTertiary), child: const Text('集中安排')),
                    const SizedBox(width: 8),
                    ElevatedButton(onPressed: count == 0 ? null : _batchMarkComplete, style: ElevatedButton.styleFrom(backgroundColor: Theme.of(context).colorScheme.primary, foregroundColor: Theme.of(context).colorScheme.onPrimary), child: const Text('完成')),
                  ],
                )
              ],
            ),
  ...
  Widget _buildMicroTaskBubble(MicroTask task) {
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        color: task.done ? (isDark ? Colors.white.withOpacity(0.02) : Colors.grey.withOpacity(0.05)) : (isDark ? const Color(0xFF1E1E1E) : Colors.white),
        borderRadius: BorderRadius.circular(20),
        border: Border.all(color: selected ? theme.colorScheme.primary : (isDark ? Colors.white10 : Colors.black12), width: selected ? 2 : 1),
        boxShadow: task.done || isDark ? [] : [BoxShadow(color: Colors.black.withOpacity(0.03), blurRadius: 10, offset: const Offset(0, 4))],
      ),
      child: Material(
        ...
        child: InkWell(
          ...
          onLongPress: () { if (!_batchMode) _confirmDeleteOne(task); },
          child: Padding(
            ...
            child: Row(
              children: [
                ...
                // 左侧图标区
                GestureDetector(
                  onTap: () { if (!_batchMode) _setDone(task, !task.done); },
                  child: Container(
                    padding: const EdgeInsets.all(10),
                    decoration: BoxDecoration(
                      color: task.done ? Colors.green.withOpacity(0.1) : theme.colorScheme.primary.withOpacity(0.1),
                      shape: BoxShape.circle,
                    ),
                    child: Icon(task.done ? Icons.check_circle_rounded : Icons.radio_button_unchecked_rounded, color: task.done ? Colors.green : theme.colorScheme.primary, size: 26),
                  ),
                ),
                ...
                Expanded(
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Text(task.title, style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700, decoration: task.done ? TextDecoration.lineThrough : null, color: task.done ? theme.colorScheme.onSurfaceVariant : theme.colorScheme.onSurface)),
                          ...
                          Container(
                            ...
                            child: Row(
                              ...
                              children: [
                                ...
                                const SizedBox(width: 4),
                                Text('${task.minutes} min', style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: theme.colorScheme.onSurfaceVariant)),
                              ],
                            ),
                          ),
                          Container(
                            ...
                            child: Row(
                              mainAxisSize: MainAxisSize.min,
                              children: [
                                Icon(Icons.tag_rounded, size: 12, color: theme.colorScheme.onSurfaceVariant),
                                const SizedBox(width: 2),
                                Text(task.tag.trim().isEmpty ? '未分类' : task.tag, style: TextStyle(fontSize: 11, fontWeight: FontWeight.w600, color: theme.colorScheme.onSurfaceVariant)),
                              ],
                            ),
                          ),
                        ],
                ...
                if (!_batchMode) ...[
                  Column(
                    ...
                    children: [
                      Text('+${_pointsFor(task)} ${_pointsUnit(context)}', style: TextStyle(fontWeight: FontWeight.w900, color: task.done ? Colors.green : theme.colorScheme.primary, fontSize: 13)),
                      const SizedBox(height: 8),
                      Row(
                        children: [
                          IconButton(icon: const Icon(Icons.edit_rounded, size: 20, color: Colors.grey), onPressed: () => _showEditMicroTaskDialog(context, task), constraints: const BoxConstraints(), padding: EdgeInsets.zero),
                          const SizedBox(width: 12),
                          IconButton(icon: const Icon(Icons.delete_outline_rounded, size: 20, color: Colors.redAccent), onPressed: () => _confirmDeleteOne(task), constraints: const BoxConstraints(), padding: EdgeInsets.zero),
                        ],
                      )
                    ],
                  ),
